$(document).ready(function () {
  ...
  function walletBalance() {
    ...
    if (host == "cryptoid.custom") {
      customCoinTicker = $("#customCoinTicker").val();
    }

    //configureGetUnspentTx();
    var explorer_addr =
      "https://chainz.cryptoid.info/blk/api.dws?key=1205735eba8c&q=getbalance&a=";
    //configureGetUnspentTx();
    var host = $("#coinjs_broadcast option:selected").val();

    $("#walletLoader").removeClass("hidden");
    coinjs.addressBalance(
      explorer_addr,
      $("#walletAddress").html(),
      function (data) {
        // if($(data).find("result").text()==1){
  ...
  $("#newPaperwalletBtn").click(function () {
    ...
    paperwallet.document.write(
      '<h2>BLK PaperWallet</h2><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Address (Share)</h3></div><div style="text-align: center;"><div id="qraddress"></div><p>' +
        $("#newBlackcoinAddress").val() +
  ...
  $("#newSegwitPaperwalletBtn").click(function () {
    ...
    var paperwallet = window.open();
    paperwallet.document.write(
      '<h2>BLK SegWit PaperWallet</h2><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Address (Share)</h3></div><div style="text-align: center;"><div id="qraddress"></div><p>' +
  ...
  function listUnspentDefault(redeem) {
    ...
    $.ajax({
      type: "GET",
      url:
        "https://chainz.cryptoid.info/blk/api.dws?key=1205735eba8c&q=unspent&active=" +
        redeem.addr,
      dataType: "json",
      error: function () {
        $("#redeemFromStatus")
          .removeClass("hidden")
          .html(
            '<span class="glyphicon glyphicon-exclamation-sign"></span> Unexpected error, unable to retrieve unspent outputs! blk test function error'
          );
      },
  ...
  function rawSubmitDefault(btn) {
    ...
    $.ajax({
      ...
      url:
        ...
        "&key=" +
        coinjs.key +
        "&setmodule=blackcoin&request=sendrawtransaction",
